...
- `index.html` — home (services, projects, writing, about, contact)
...
The visual system retains the dark background, cyan/brass accents, and Geist typography. The homepage opens with interactive artwork and direct project links, followed by a swipeable project gallery, writing, compact service disclosures, and contact. Work pages use matching SVG drawings and native cross-document transitions where supported. Case studies and essays link onward to the next item; articles expose a heading navigation rail on wide screens. The writing index has topic filters.

`assets/pattern.css` and `assets/pattern.js` own the canvas artwork, palette, optional alternate appearance, and PNG export. `assets/flow.css` and `assets/flow.js` own layout, project drawings, gallery input, filtering, and navigation. Public controls use functional labels; the fantasy references are visual. The header star changes the appearance across pages and stores that preference locally when available. Canvas rendering settles after interaction and pauses offscreen or in background tabs. Reduced-motion preferences disable animation and shared-image transitions. No graphics libraries or runtime API integration are added. The original small wheel still turns through `assets/wheel.js`. Without JavaScript, content, native project links, scrolling gallery, and service disclosures remain usable. Shared off-site wording and claim sources live in `copy/brand.md`; those drafts have not been posted to external profiles.
